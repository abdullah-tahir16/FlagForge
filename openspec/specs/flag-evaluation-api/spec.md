...
The system SHALL allow an SDK client to evaluate one boolean flag by flag key for the SDK key's environment, including ordered targeting rules and deterministic percentage rollout behavior.

#### Scenario: Enabled boolean flag with full rollout returns configured value
- **WHEN** an SDK client evaluates an enabled boolean flag with no matching targeting rule and rollout percentage `100` for its environment
...
- **THEN** the system returns `false` with a disabled reason before applying targeting or rollout logic
...
- **WHEN** an SDK client evaluates an enabled boolean flag and the first matching targeting rule returns `true` or `false`
- **THEN** the system returns that rule result value before applying percentage rollout logic

#### Scenario: First matching targeting rule wins
- **WHEN** multiple enabled targeting rules match the same evaluation context
- **THEN** the system returns the result value from the matching rule with the lowest sort order

#### Scenario: No matching targeting rule falls back to rollout
- **WHEN** an SDK client evaluates an enabled boolean flag and no targeting rule matches
...
The system SHALL allow an SDK client to evaluate all boolean flags for the SDK key's environment, including ordered targeting rules and deterministic percentage rollout behavior.
...
- **WHEN** an SDK client requests all flags with context attributes matching targeting rules
- **THEN** the system evaluates each flag's ordered targeting rules independently and includes each resulting value and reason metadata
...
- **WHEN** evaluation returns a value because a targeting rule matched
